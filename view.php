...
if ($download == '' && $form = data_submitted() && has_capability('mod/booking:choose', $context)) {
    ...
    if (!empty($answer)) {
        ...
        if ($bookingdata->user_submit_response($USER)) {
            $contents = html_writer::tag('p', get_string('bookingsaved', 'booking')); // Patch: HA#006 mod_booking appearance tweaks
            if ($booking->settings->sendmail) {
                $contents .= html_writer::tag('p', get_string('mailconfirmationsent', 'booking') . "."); // Patch: HA#006 mod_booking appearance tweaks
            }
